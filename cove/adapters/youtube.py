...
class YouTubeAdapter(Adapter):
    ...
    def enumerate(self, target: str, settings: dict | None = None) -> list[Proposal]:
        ...
        try:
            ...
            # Fall back to a single opaque video Proposal; the video engine will try it.
            return [Proposal.single(title=target, url=target, kind="video",
        ...
        if entries:
            for e in entries:
                ...
                out.append(Proposal.single(
                    title=e.get("title") or url, url=url, kind="video",
                    quality=quality, library=library,
                    meta={"channel": e.get("channel") or e.get("uploader")}))
        else:
            out.append(Proposal.single(title=(info or {}).get("title") or target,
                                       url=target, kind="video", quality=quality,
                                       library=library))
        return out
